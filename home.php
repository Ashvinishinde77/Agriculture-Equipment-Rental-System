        ...
        <div class="container-fluid">
          ...
          <div class="row g-3">
			...
			<?php
				  include('./zel_admin/conn.php');
				  error_reporting(E_ALL);
				  $sql = "SELECT p.pid, p.imgpath, p.pcode, p.pname, r.price, sc.sc_name FROM rate r, products p, category c, sub_category sc WHERE p.pid=r.pid AND c.cid=sc.cid AND sc.sc_id=p.sc_id AND p.status=1 ORDER BY p.pid DESC LIMIT 32";
				  $result = $conn->query($sql);
				  if ($result->num_rows > 0) {
					  while($row = $result->fetch_assoc()) {
