...
if ($result->num_rows > 0) {
 ...
 if($sc_name=="Farming Equipments"){$price= $price."/Hrs";$cartlbl="Hire On Rent";}	
else{$cartlbl="Add To Cart";} 
}
else{
        ...
        <div class="specification bg-white mb-3 py-3">
          <div class="container d-flex justify-content-between">
            <div class="p-title-price">
              ...
              <p class='sale-price1'> Delivery/Operator Charge: <i class='lni lni-rupee'></i> <?php echo $delivery_charges;?><span></span></p>
		...
		<div class="cart-form-wrapper bg-white mb-3 py-3">
          <div class="container">		  
            <a href='cart.php?action=add&quantity=1&pid=<?php echo $pid; ?>'>              
              <button class="btn btn-danger ml-3" type="button"><?php echo $cartlbl;?></button>
            </a>
          </div>
        </div>
		      <!-- Top Products-->
        ...
        <div class="container-fluid">
          ...
          <div class="row g-3">
			...
			<?php
				  include('./zel_admin/conn.php');
				  error_reporting(E_ALL);
				  $sql = "SELECT p.pid, p.imgpath, p.pcode, p.pname, r.price,sc.sc_name FROM rate r, products p, category c, sub_category sc WHERE p.pid=r.pid AND c.cid=sc.cid AND sc.sc_id=p.sc_id AND p.pid!=$pid AND p.sc_id=$sc_id AND p.status=1 ORDER BY p.pid DESC LIMIT 4";
				  $result = $conn->query($sql);
				  if ($result->num_rows > 0) {
					  while($row = $result->fetch_assoc()) {
						echo"<div class='col-6 col-md-4 col-lg-3'>
						  <div class='card top-product-card'>
							...
							if($row['sc_name']=="Farming Equipments"){echo"<p class='sale-price'> Price: <i class='lni lni-rupee'></i> ".$row['price']." /Hrs<span></span></p>";}
							  ...
							  echo"<p class='sale-price'>".$row['pcode']."</p>							  						  
