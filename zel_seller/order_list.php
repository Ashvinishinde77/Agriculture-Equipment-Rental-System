...
if (isset($_POST['submitstatus'])){
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT o.order_id, o.order_status, o.order_date, e.euid, e.euname, e.eumob, e.address FROM orders o, order_item oi, products p, seller s, end_user e WHERE e.euid=o.euid AND s.sid=p.sid AND p.pid=oi.pid AND o.order_id=oi.order_id AND s.sid=$user_id AND order_status=$order_status GROUP BY o.order_id ORDER BY o.order_id DESC";
	}
}
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$sql = "SELECT o.order_id, o.order_status, o.order_date, e.euid, e.euname, e.eumob, e.address FROM orders o, order_item oi, products p, seller s, end_user e WHERE e.euid=o.euid AND s.sid=p.sid AND p.pid=oi.pid AND o.order_id=oi.order_id AND s.sid=$user_id AND order_status=1 AND order_date BETWEEN '$datefrom' AND '$dateto' GROUP BY o.order_id ORDER BY o.order_id DESC";
	}
}
else{
	$sql = "SELECT o.order_id, o.order_status, o.order_date, e.euid, e.euname, e.eumob, e.address FROM orders o, order_item oi, products p, seller s, end_user e WHERE e.euid=o.euid AND s.sid=p.sid AND p.pid=oi.pid AND o.order_id=oi.order_id AND s.sid=$user_id AND order_status=1 GROUP BY o.order_id ORDER BY o.order_id DESC";
}
